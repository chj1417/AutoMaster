...
class LoadWin(object):
    def setupUi(self, Dialog):
        ...
        self.loginbtn.clicked.connect(lambda :self.clicklogin(Dialog))
    ...
    def clicklogin(self,Dialog):
        g, u, p = self.group.currentIndex(), self.user.text(), self.pwd.text()
        groupli=['Admin','Operator','Viewer']
        if JCheck.check().gup(g,u,p):
            # 退出对话框
            Dialog.accept()
            logging.info('Longin %s=%s' % (groupli[g], u))
        else:
            QMessageBox.warning(None, ("Warning"), '用户名或密码错误', QMessageBox.StandardButton(QMessageBox.Ok))
            self.pwd.setText('')
            self.user.setFocus()
